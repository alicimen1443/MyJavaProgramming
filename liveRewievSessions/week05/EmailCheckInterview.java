...
public class EmailCheckInterview {
    public static void main(String[] args) {
        ...
        if(atSignIndex<1 //  0 (no character before @ sign) or -1 (there is no @ sign)
                ...
                ||atSignIndex!=email.lastIndexOf("@")) // there is only one @ sign, or if they are not equal means more @ sign
        {
            result = false;
        }
        if( lastDotIndex < 1 // 0 (no character before dot sign) or -1 (there is no dot sign)
                || lastDotIndex == email.length()-1 // checking if it is the last character
                || atSignIndex > lastDotIndex  // checking if @ sign is before dot sign
                || atSignIndex+1 == lastDotIndex){ // checking if there is a character between @ sign and dot sign
            result = false;
        }
